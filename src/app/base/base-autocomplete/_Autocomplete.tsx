"use client";
import { BaseAutocomplete } from "~/ui/components";
...
export function Autocomplete() {
  return (
    <BaseAutocomplete.Root items={tags}>
      <label className="flex flex-col gap-1 text-sm leading-5 font-bold">
        Search tags
        <BaseAutocomplete.Input
          placeholder="e.g. feature"
          className="h-10 w-[16rem] border border-gray-200 pl-3.5 text-base
            font-normal focus:outline-2 focus:-outline-offset-1
            focus:outline-blue-800 md:w-[20rem]"
        />
      </label>
        ...
        <BaseAutocomplete.Positioner>
          <BaseAutocomplete.Popup>
            ...
            <BaseAutocomplete.List>
              {(tag: Tag) => (
                <BaseAutocomplete.Item key={tag.id} value={tag}>
                  {tag.value}
